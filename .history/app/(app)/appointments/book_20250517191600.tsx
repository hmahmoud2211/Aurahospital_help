...
export default function BookAppointmentScreen() {
  ...
  const handleBookAppointment = async () => {
    ...
    const appointmentData = {
      patient: Number(user?.id || 1),
      doctor: Number(selectedDoctor.id),
      date: selectedDate,
      time: selectedTime,
      duration: 30,
    ...
    // @ts-expect-error: Backend expects patient and doctor as numbers (IDs)
    const success = await bookAppointment(appointmentData);
    
    if (success) {
      // Force refresh appointments for this user
      if (user) {
              ...
              {specialties.map((item) => (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    // Filter doctors by specialty in a real app
                  }}
                >
                  <Text
                    style={[
          ...
          >
            ...
            <View style={styles.doctorInfo}>
              <Text style={styles.doctorName}>
                {selectedDoctor ? (Array.isArray(selectedDoctor.name) ? selectedDoctor.name[0]?.text : selectedDoctor.name) : ''}
              </Text>
              <Text style={styles.doctorSpecialty}>
                {selectedDoctor ? (Array.isArray(selectedDoctor.specialty) ? selectedDoctor.specialty[0] : selectedDoctor.specialty) : ''}
              </Text>
            </View>
            <ChevronDown size={20} color={Colors.textSecondary} />
          </TouchableOpacity>
                ...
                >
                  ...
                  <View style={styles.doctorItemInfo}>
                    <Text style={styles.doctorItemName}>
                      {Array.isArray(doctor.name) ? doctor.name[0]?.text : doctor.name}
                    </Text>
                    <Text style={styles.doctorItemSpecialty}>
                      {Array.isArray(doctor.specialty) ? doctor.specialty[0] : doctor.specialty}
                    </Text>
                  </View>
                </TouchableOpacity>
              ))}
